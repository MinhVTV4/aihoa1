 ...
 * This version implements the "Energy Transition" concept.
...
function runAnimation(plan) {
    ...
    plan.animationSteps.forEach((step, index) => {
        ...
        if (step.type === 'move_to_center') {
            stepTimeline.to(camera.position, { z: 25, duration: 2, ease: "power2.inOut"}, 0);
            reactantGroups.forEach(group => {
                ...
                stepTimeline.to(group.rotation, { x: '+=6', y: '+=6', duration: 2, ease: "power1.inOut" }, 0);
            });
            ...
            // --- Phase 1: Collision & Activation ---
            stepTimeline.addLabel("collision", "+=0");
            stepTimeline.to(camera.position, { z: 18, duration: DURATION * 0.5, ease: "power3.in" }, "collision");
            
            const shake = { strength: 0.15 };
            stepTimeline.to(shake, {
                strength: 0, duration: 0.5,
                onUpdate: () => {
                    camera.position.x += (Math.random() - 0.5) * shake.strength;
                    camera.position.y += (Math.random() - 0.5) * shake.strength;
                }
            }, "collision+=" + (DURATION * 0.5 - 0.25));

            stepTimeline.add(() => {
                const flash = new THREE.PointLight(0xffffff, 100, 30, 2);
                flash.position.copy(REACTION_CENTER);
                flash.userData.isEffect = true;
                scene.add(flash);
                gsap.to(flash, { intensity: 0, duration: 0.6, ease: "power2.out", onComplete: () => scene.remove(flash) });
            }, "collision+=" + (DURATION * 0.5 - 0.05));

            // --- Phase 2: Destruction & Chaos ---
            stepTimeline.addLabel("chaos", "collision+=" + DURATION * 0.5);
            const detachedAtoms = [];
            reactantGroups.forEach(group => {
                group.userData.moleculeData.bondMeshes.forEach(bond => {
                    stepTimeline.to(bond.material, { opacity: 0, duration: 0.3 }, "chaos");
                });
            ...
            detachedAtoms.forEach(atom => {
                stepTimeline.to(atom.position, {
                    x: REACTION_CENTER.x + (Math.random() - 0.5) * 12,
                    y: REACTION_CENTER.y + (Math.random() - 0.5) * 12,
                    z: REACTION_CENTER.z + (Math.random() - 0.5) * 12,
                    duration: DURATION * 0.7, ease: "power2.out"
                }, "chaos");
                stepTimeline.to(atom.material.emissive, { r: 0.8, g: 0.8, b: 0.4, duration: DURATION * 0.5, ease: "power2.in" }, "chaos");
            });
            ...
            plan.products.forEach(p => {
                for (let j = 0; j < Math.max(1, p.count) * baseMultiplier; j++) {
                    ...
                    productGroup.traverse(child => {
                        if (child.isMesh) {
                            stepTimeline.to(child.material, { opacity: 1, duration: DURATION * 0.7 }, `reform+=${DURATION*0.3}`);
                        }
                    });
            ...
            stepTimeline.to(atomPool.map(a=>a.material), {opacity: 0, duration: 0.5, onComplete: ()=> atomPool.forEach(a=>scene.remove(a))}, 'reform');
            ...
            stepTimeline.add(() => {
                if (solutionContainer) {
                    ...
                    gsap.to(solutionContainer.material.color, { r: finalColor.r, g: finalColor.g, b: finalColor.b, duration: duration, ease: "power1.inOut" });
                    gsap.to(solutionContainer.material, { opacity: finalOpacity, duration: duration, ease: "power1.inOut" });
                }
            });
        }
